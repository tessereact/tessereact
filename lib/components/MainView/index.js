...
var MainView = function (_React$Component) {
  ...
  _createClass(MainView, [{
    ...
    value: function componentWillMount() {
      ...
      var url = '/api/read-snapshots';
      var startDate = Date.now();

      Promise.all([
      // Get config from server
      window.__tessereactConfig ? Promise.resolve(null) : (0, _requests.getJSON)('/api/config').then(function (config) {
      ...
      this.props.data.some(function (_ref) {
        ...
        return css;
      }) ? (0, _requests.getJSON)('/api/css').then(function (_ref2) {
